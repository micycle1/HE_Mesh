...
public class WB_QuickHull3D
{
    ...
    public WB_QuickHull3D(final Collection<? extends WB_Coord> points) throws IllegalArgumentException {
        ...
        this.faces = new Vector<Face>(16);
        this.horizon = new Vector<HalfEdge>(16);
        this.newFaces = (FastList<Face>)new FastList();
        this.unclaimed = new VertexList();
        this.claimed = new VertexList();
        this.points = (List<WB_Coord>)new FastList();
        for (final WB_Coord point : points) {
            this.points.add((WB_Coord)new WB_Point(point));
        }
        this.build(false);
    }
    ...
    public void triangulate() {
        ...
        for (final Face face : this.faces) {
            if (face.mark == 1) {
                face.triangulate((List<Face>)this.newFaces, minArea);
            }
        }
    ...
    protected void createInitialSimplex() throws IllegalArgumentException {
        ...
        double maxSqr = 0.0;
        u01.subSelf((WB_Coord)vtx[0].pos);
        u01.normalizeSelf();
        for (int j = 0; j < this.numPoints; ++j) {
            ...
            xprod.set(u01.yd() * diff2.zd() - u01.zd() * diff2.yd(), u01.zd() * diff2.xd() - u01.xd() * diff2.zd(), u01.xd() * diff2.yd() - u01.yd() * diff2.xd());
            final double lenSqr = xprod.getSqLength3D();
            if (lenSqr > maxSqr && p != vtx[0] && p != vtx[1]) {
                maxSqr = lenSqr;
                vtx[2] = p;
                nrml.set((WB_Coord)xprod);
            }
        }
        ...
        nrml.normalizeSelf();
        double maxDist = 0.0;
        final double d0 = nrml.dot((WB_Coord)vtx[2].pos);
        for (int k = 0; k < this.numPoints; ++k) {
            final Vertex p2 = this.pointBuffer[k];
            final double dist = Math.abs(nrml.dot((WB_Coord)p2.pos) - d0);
        ...
        if (nrml.dot((WB_Coord)vtx[3].pos) - d0 < 0.0) {
            ...
            if (v != vtx[0] && v != vtx[1] && v != vtx[2]) {
                if (v != vtx[3]) {
                    ...
                    for (int k2 = 0; k2 < 4; ++k2) {
                        final double dist2 = tris[k2].distanceToPlane((WB_Coord)v.pos);
                        if (dist2 > maxDist) {
                            maxFace = tris[k2];
                            maxDist = dist2;
                        }
    ...
    public List<WB_Coord> getVertices() {
        final List<WB_Coord> vtxs = (List<WB_Coord>)new FastList();
    ...
    protected double oppFaceDistance(final HalfEdge he) {
        return he.face.distanceToPlane((WB_Coord)he.opposite.face.getCentroid());
    }
    ...
    protected void calculateHorizon(final double eyePntx, final double eyePnty, final double eyePntz, HalfEdge edge0, final Face face, final Vector<HalfEdge> horizon) {
    ...
    protected void addPointToHull(final Vertex eyeVtx) {
        ...
        this.newFaces.clear();
        this.addNewFaces((List<Face>)this.newFaces, eyeVtx, this.horizon);
        ...
        this.resolveUnclaimedPoints((List<Face>)this.newFaces);
    }
    ...
    static class Face
    {
        ...
        protected WB_Vector computeCentroid() {
            ...
            do {
                ...
                this.centroid.addSelf((WB_Coord)v.pos);
                he = he.next;
            } while (he != this.he0);
            return this.centroid.divSelf((double)this.numVerts);
        }
        ...
        protected void computeNormal(final WB_Vector normal, final double minArea) {
            ...
            if (this.area < minArea) {
                ...
                final WB_Vector u = p2.pos.sub((WB_Coord)p3.pos).divSelf(lenMax);
                final double dot = normal.dot((WB_Coord)u);
                normal.addMul(-dot, (WB_Coord)u);
        ...
        private void computeNormalAndCentroid() {
            ...
            this.planeOffset = this.normal.dot((WB_Coord)this.centroid);
            int numv = 0;
            HalfEdge he = this.he0;
            do {
                ++numv;
        ...
        protected static Face create(final FastList<Vertex> vtxArray, final int[] indices) {
            ...
            for (final int indice : indices) {
                final HalfEdge he = new HalfEdge((Vertex)vtxArray.get(indice), face);
                if (hePrev != null) {
                    he.setPrev(hePrev);
                    hePrev.setNext(he);
                }
        ...
        protected int mergeAdjacentFace(final HalfEdge hedgeAdj, final Face[] discarded) {
            ...
            for (hedgeOppNext = hedgeOpp.next; hedgeAdjPrev.oppositeFace() == oppFace; hedgeAdjPrev = hedgeAdjPrev.prev, hedgeOppNext = hedgeOppNext.next) {}
    ...
    static class HalfEdge
    {
        ...
        protected double length() {
            if (this.tail() != null) {
                return WB_GeometryOp.getDistance3D((WB_Coord)this.head().pos, (WB_Coord)this.tail().pos);
            }
            return -1.0;
        }
        ...
        protected double lengthSquared() {
            if (this.tail() != null) {
                return WB_GeometryOp.getSqDistance3D((WB_Coord)this.head().pos, (WB_Coord)this.tail().pos);
            }
            return -1.0;
        }
    }
