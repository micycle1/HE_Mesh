...
public class Polygon
{
    ...
    private void initializate() {
        ...
        for (int i = 0; i < this._pointsKeys.length; ++i) {
            final int id = this._pointsKeys[i];
            ...
            if (p.compareTo(pnext) > 0 && pprev.compareTo(p) > 0) {
                ...
                final double area = Poly2TriUtils.orient2d(new double[] { pprev.x, pprev.y }, new double[] { p.x, p.y }, new double[] { pnext.x, pnext.y });
    ...
    public boolean triangulation() {
        if (!this.partition2Monotone()) {
            return false;
        }
        if (!this.searchMonotones()) {
            return false;
        }
        for (int i = 0; i < this._mpolys.size(); ++i) {
            this.triangulateMonotone(this._mpolys.get(i));
        }
