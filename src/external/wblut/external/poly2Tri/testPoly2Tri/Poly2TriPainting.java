...
public class Poly2TriPainting extends JPanel
{
    ...
    protected int[] getPoint(final double x, final double y) {
        ...
        final int[] point = { (int)Math.round(realCenter[0] - center[0] * xSize + x * xSize), (int)Math.round(realCenter[1] + center[1] * ySize - y * ySize) };
        return point;
    }
    ...
    public void paint(final Graphics g) {
        for (int i = 0; i < this.polygons.size(); ++i) {
            final double[] xy = this.polygons.get(i);
