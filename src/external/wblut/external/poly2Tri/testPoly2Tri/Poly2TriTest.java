...
public class Poly2TriTest {
	...
	static {
		...
		Poly2TriTest.tstfs = new ArrayList<>();
		Poly2TriTest.name = "";
	}
	...
	public static void doTriangulation() {
		...
		double[] xy2 = { 0.0, 0.0 };
		double[] xy3 = { 0.0, 0.0 };
		for (final ArrayList<Integer> t : Poly2TriTest.triangles) {
			for (int j = 0; j < 3; ++j) {
	...
	public static Double getNumber(final String str, final int[] index) {
		...
		while (index[0] < str.length()
				&& (str.charAt(index[0]) == 'e' || str.charAt(index[0]) == 'E' || str.charAt(index[0]) == '+' || str.charAt(index[0]) == '-'
						|| str.charAt(index[0]) == '.' || (str.charAt(index[0]) >= '0' && str.charAt(index[0]) <= '9'))) {
	...
	public static boolean loadBDMFile(final String fileName) {
		...
		try {
			...
			while (fr.ready()) {
				...
				if ((line == null) || (line.length() == 0) || line.charAt(0) < '0' || line.charAt(0) > '9') {
					continue;
				}
		...
		for (int j = 0; j < Poly2TriTest.numContours; ++j) {
			...
			for (double[] element : verticesAL) {
				Poly2TriTest.vertices[k++] = element;
			}
		}
		return true;
	}
	...
	public static void main(final String[] args) {
		...
		Poly2TriTest.vertices = new double[][] { { 0.0, 0.0 }, { 9.0, 0.0 }, { 3.0, 8.0 }, { 2.0, 1.0 }, { 2.0, 2.0 }, { 3.0, 1.0 },
				{ 6.0, 2.0 }, { 6.0, 1.0 }, { 5.0, 2.0 } };
		...
		boolean next = false;
		boolean all = false;
		int option = JOptionPane.showConfirmDialog(Poly2TriTest.tstfs.get(Poly2TriTest.tstfs.size() - 1),
				"Do you want to do ALL bdm files triangulation?");
		all = (option == 0);
		for (String bdmFile : bdmFiles) {
			if (!all) {
				next = false;
				option = JOptionPane.showConfirmDialog(Poly2TriTest.tstfs.get(Poly2TriTest.tstfs.size() - 1),
						"Do triangulation of " + bdmFile + "?");
				switch (option) {
					case 2 : {
						end = true;
						break;
			...
			if (loadBDMFile(String.valueOf(prefix) + bdmFile)) {
				Poly2TriTest.name = bdmFile;
				doTriangulation();
			} else {
				JOptionPane.showMessageDialog(Poly2TriTest.tstfs.get(Poly2TriTest.tstfs.size() - 1), "Failed to load " + bdmFile);
			}
		}
	}
}
